...
typedef struct _LoginResponsePkg LoginResponsePkg;
...
#define LOGIN__RESPONSE__PKG__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&login__response__pkg__descriptor) \
    , 0, NULL, 0 }
...
struct  _BodyPkg
{
  ...
  LoginResponsePkg *login_res_pkg;
};
...
typedef void (*LoginPkg_Closure)
                 (const LoginPkg *message,
                  void *closure_data);
typedef void (*LoginResponsePkg_Closure)
                 (const LoginResponsePkg *message,
                  void *closure_data);
...
extern const ProtobufCMessageDescriptor login__response__pkg__descriptor;
